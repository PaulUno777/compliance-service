...
@Injectable()
export class SanctionedProvider {
  ...
  mapSearchResult(result: any, fullName: string): sanctionedSearchOutput {
    const entity = {
      id: result.entity.id,
    ...
    const names = this.searchTools.getAllNames(result);
    const score = this.searchTools.setPercentage(names, fullName);

    return { entity, score };
  }

  // Clean up by mapping and removing duplicate data
  ...
  mapExcelData(array: any[], searchInput: string): any[] {
    this.logger.log('----- Mapping data to Excel file');
    const cleanData = [];
